...
#   2. 39-312 (2009-11-17): seq -> printf

scriptName="$0"
#INCLUDE_PATH="`echo $0 | sed -e 's/\/.*$//'`/../include"
INCLUDE_PATH="../../include"
CFLAGS="-O2 -Wall"
CXXFLAGS="-O2 -Wall -I $INCLUDE_PATH -I ../$INCLUDE_PATH"
FPCFLAGS="-O3 -FE. -v0ewn -Sd -Fu../$INCLUDE_PATH -Fi../$INCLUDE_PATH -Fu$INCLUDE_PATH -Fi$INCLUDE_PATH"
...
source_compile()
{
  ...
  targetFile="$3"
  if [ ${#*} -lt 3 ]; then
    targetFile=`echo "$sourceFile" | sed -e "s/\\.[^.]*$//"`
  fi
  result="$targetFile"
  case "$language" in
    "c")
      tsh_information "information" "gcc $CFLAGS -o \"$targetFile\" -x c \"$sourceFile\""
      gcc $CFLAGS -o "$targetFile" -x c "$sourceFile" || tsh_information "error" "compile failed"
    ;;
    "cpp")
      tsh_information "information" "g++ $CXXFLAGS -lstdc++ -o \"$targetFile\" -x c++ \"$sourceFile\""
      g++ $CXXFLAGS -lstdc++ -o "$targetFile" -x c++ "$sourceFile" || tsh_information "error" "compile failed"
    ;;
    "dpr")
      tsh_information "information" "fpc $FPCFLAGS -o\"$targetFile\" \"$sourceFile\" && cp \"${targetFile}\" \"${targetFile}.exe\""
      fpc $FPCFLAGS -o"$targetFile" "$sourceFile" || tsh_information "error" "compile failed"
    ;;
    "java")
      targetFile=`echo "$sourceFile" | sed -e "s/\\.[^.]*$//"`".class"
      tsh_information "information" "javac \"$sourceFile\""
      javac "$sourceFile" || tsh_information "error" "compile failed"
      result="$targetFile"
    ;;
    "pas")
      tsh_information "information" "fpc $FPCFLAGS -o\"$targetFile\" \"$sourceFile\" && cp \"${targetFile}\" \"${targetFile}.exe\""
      fpc $FPCFLAGS -o"$targetFile" "$sourceFile" && cp $targetFile{,.exe} || tsh_information "error" "compile failed"
    ;;
    "pl")
...
source_run()
{
  ...
  if [ "$language" == "java" ]; then
    ...
    if [ "$outputFile" != "" ]; then
      java -Xmx256M -Xss128M -cp "$classPath" "$className" ${*:5} > "$outputFile"
    elif [ "$inputFile" != "" ]; then
      java -Xmx256M -Xss128M -cp "$classPath" "$className" ${*:5} < "$inputFile"
    else
      java -Xmx256M -Xss128M -cp "$classPath" "$className" ${*:5}
    fi
  elif [ "$language" == "pl" ]; then
    if [ "$outputFile" != "" ]; then
      perl $binaryFile ${*:5} > "$outputFile"
    elif [ "$inputFile" != "" ]; then
      perl $binaryFile ${*:5} < "$inputFile"
    else
      perl $binaryFile ${*:5}
    fi
  elif [ "$language" == "pas" ]; then
    if [ -e "$binaryFile.exe" ] ; then
      bb="$binaryFile.exe"
    else
      bb="$binaryFile"
    fi
    ...
    if [ "$outputFile" != "" ]; then
      "./$binaryFile" ${*:5} > "$outputFile"
    elif [ "$inputFile" != "" ]; then
      "./$binaryFile" ${*:5} < "$inputFile"
    else
      "./$binaryFile" ${*:5}
    fi
  fi
...
{
  result=()
#  for file in `seq -w 00 99` `seq -w 000 999`; do # TODO: optimize this
...
do_check()
{
  ...
  solutionLanguage="$3"
  # WTFTODO: for
  if find_source "../check"; then
    checkerName="../check.$result"
    checkerLanguage="$result"
    checkerBinary="../check"
  elif find_source "../checker"; then
    checkerName="../checker.$result"
    checkerLanguage="$result"
    checkerBinary="../checker"
  elif find_source "../check_$problemName"; then
    checkerName="../check_$problemName.$result"
    checkerLanguage="$result"
    checkerBinary="../check_$problemName"
  else
  ...
  for testNumber in ${tests[*]}; do
    ...
    source_run "$checkerBinary" "$checkerLanguage" "" "" "$testNumber" "$problemName.out" "$testNumber.a" || tsh_information "error" "check failed on test [$testNumber]"
  done
...
# This is still not enough good, replaced for compatibility.
    ...
    solutionSuffix="$1"
    if [ "$1" == "" ] && [ -f "../problem.properties" ]; then
      solutionSuffix="`cat ../problem.properties | grep solution | sed -e 's/solution *= *\([a-z,A-Z,0-9,_,.]*\) *$/\1/'`"
    fi
    ...
    for testNumber in ${tests[*]}; do
      ...
      cp "$testNumber" "$problemName.in"
      source_run "$solutionBinary" "$solutionLanguage" || tsh_information "error" "solution failed on test [$testNumber]"
      cp "$problemName.out" "$testNumber.a"
    done
    echo "ok"
    rm --force "$problemName."{in,out}
...
t_clean()
{
  ...
  for (( currentProblem = 0; currentProblem < ${#problems[*]}; currentProblem++ )); do
    ...
    rm --force tests/*.{exe,dcu,ppu,o,obj,class,hi,manifest}
