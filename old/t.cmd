...
if "%success%" == "true" (
  ...
  echo BUILD FAILED FOR PROBLEM %problem.id%. See build.log.
  exit /b 1
)
...
if "%testnos%" == "" (
    for /l %%i in (0,1,9) do for /l %%j in (0,1,9) do (
        call :check_sub %%i%%j
        rem if not "!success!" == "true" goto check_exit_loop
    )
) else (
...
if "%success%" == "true" (
  ...
  echo CHECK FAILED FOR PROBLEM %problem.id%. See check.log.
  exit /b 1
)
...
if not exist tests (
  ...
  echo Scanning %CD%
  call :rec_sub %*
  if exist contest.properties call :challenge
  exit /b
)
...
if not exist check.exe (
  echo XML FAILED FOR PROBLEM %problem.id% -- must build first, check.exe is missing.
  exit /b 1
)
...
call :load_properties contest.properties contest
call :challenge_print_header > challenge.xml

for /d %%d in (*) do if /i "%%d" neq "tests" if /i "%%d" neq ".svn" if /i "%%d" neq "CVS" if /i "%%d" neq "__pcms" (
  pushd %%d
  if exist problem.properties (
    set problem=%%d
    call :load_problem_properties
    call :challenge_print_problem >> ..\challenge.xml
  )
  popd
)

call :challenge_print_footer >> challenge.xml

exit /b

:challenge_print_header
...
:challenge_print_problem

echo     ^<problem-ref alias = "%problem.alias%" id = "%problem.alias%" problem-id = "%prefix%%problem.id%" name = "%problem.name%" /^>

exit /b

:challenge_print_footer
...
if not exist check.exe (
  echo PCMS FAILED FOR PROBLEM %problem.id% -- must build first, check.exe is missing.
  exit /b 1
)
...
copy check.exe %dir%\__pcms\%problem.id%\ > nul
copy check.dpr %dir%\__pcms\%problem.id%\ > nul
...
if not exist check.exe (
  echo ARCHIVE FAILED FOR PROBLEM %problem.id% -- must build first, check.exe is missing.
  exit /b 1
)
...
if "[%~1]" == "[]" (
    echo Usage: t banner ^<file name^> ^<author^>
    exit /b 1
)
set file=%~1
...
if "%file:~-2%" == ".c"     set banner.language=c
if "%file:~-4%" == ".cpp"   set banner.language=c
if "%banner.language%" == "" (
...
rem call :textLine ""
rem call :textLine "Original idea         %problem.idea%"
rem call :textLine "Problem statement     %problem.statement%"
rem call :textLine "Testset               %problem.testset%"
call :solidLine 
if "%file:~1,5%" == "check" (
...
if /i "%srcfile:~-4%" == ".cpp" (
  call cl /EHsc -I%~dp0..\testlib\ %srcfile%
  exit /b
)
if /i "%srcfile:~-2%" == ".c" (
...
if exist %wrapper% (
  set wrapper=%wrapper% -q -x -t 2s -s run.dat
) else (
  set wrapper=
)
goto run_main
...
if /i "%srcfile:~-5%" == ".java" (
  if "%wrapper%" == "" (
    call %wrapper% java -Xss64m -cp ".;%~dp0..\..\testlib" -ea %srcfile:~0,-5% %cmdargs%
  ) else (
    call %wrapper% -Xifce java -Xss64m -cp ".;%~dp0..\..\testlib" -ea %srcfile:~0,-5% %cmdargs%
  )
  exit /b
)
if /i "%srcfile:~-4%" == ".class" (
  if "%wrapper%" == "" (
    ...
    call %wrapper% -Xifce java -Xss64m -cp . -ea %srcfile:~0,-5% %cmdargs%
  )
  exit /b
)
if /i "%srcfile:~-4%" == ".cpp" (
...
if /i "%srcfile:~-3%" == ".ml" (
  call %wrapper% "%srcfile:~0,-3%.exe" %cmdargs%
  exit /b
)
if /i "%srcfile:~-4%" == ".cmd" (
...
for %%i in (java dpr cpp c cmd exe) do if exist %result%.%%i (
  set result=%result%.%%i
  exit /b
)
exit /b
...
if not [%problem.alias%] == [] goto problem_alias
