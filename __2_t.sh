...
CFLAGS="-O2 -Wall"
if test $WINDIR ; then
  CFLAGS="$CFLAGS -Wl,--stack=67108864"
fi
CXXFLAGS="$CFLAGS -I $INCLUDE_PATH -I../$INCLUDE_PATH -D__T_SH__"
...
source_compile()
{
  ...
  case "$language" in
    ...
    ;;
    "dpr")
      tsh_information "information" "fpc $FPCFLAGS -o\"$targetFile\" \"$sourceFile\""
    ...
    ;;
    "pas")
      tsh_information "information" "fpc -O3 -FE. -v0ewn -o\"$targetFile\" \"$sourceFile\""
      fpc $FPCFLAGS -o"$targetFile" "$sourceFile" || tsh_information "error" "compile failed"
      cp $targetFile{,.exe}
    ;;
    "pl")
...
source_run()
{
  ...
  if [ "$language" == "java" ]; then
    ...
    if [ "$outputFile" != "" ]; then
      java -Xmx256M -cp "$classPath" "$className" ${*:5} > "$outputFile"
    elif [ "$inputFile" != "" ]; then
      java -Xmx256M -cp "$classPath" "$className" ${*:5} < "$inputFile"
    else
      java -Xmx256M -cp "$classPath" "$className" ${*:5}
    fi
...
t_build()
{
  ...
  for (( currentProblem = 0; currentProblem < ${#problems[*]}; currentProblem++ )); do
    ...
    if [ -f "doall.sh" ]; then
      ...
      "sh" "doall.sh" || tsh_information "error" "doall.sh failed"
